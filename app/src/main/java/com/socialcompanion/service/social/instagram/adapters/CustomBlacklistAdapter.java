...
public class CustomBlacklistAdapter extends ArrayAdapter<InstagramUserObject> implements View.OnClickListener{
    ...
    public CustomBlacklistAdapter(ArrayList<InstagramUserObject> data, Context context) {
        super(context, R.layout.fragment_user_blacklist, data);
        this.dataSet = data;
    ...
    public void onClick(View v) {
        ...
        HashSet<String> hashSet = (HashSet<String>) sharedPref.getStringSet("blacklist", new HashSet<String>());

        if(!sharedPref.getStringSet("blacklist", new HashSet<String>()).contains(dataModel.getUsername())){
            hashSet.add(dataModel.getUsername());
            ((ImageView)v.findViewById(R.id.imageView3)).setImageResource(R.drawable.remove_red);
        }
        else{
            hashSet.remove(dataModel.getUsername());
            ((ImageView)v.findViewById(R.id.imageView3)).setImageResource(R.drawable.add_red);
        }
        editor.remove("blacklist");
        editor.apply();
        editor.putStringSet("blacklist", hashSet);
        editor.apply();
    }
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        if (convertView == null) {
            ...
            convertView = inflater.inflate(R.layout.fragment_user_blacklist, parent, false);
        ...
        if(sharedPref.getStringSet("blacklist", new HashSet<String>()).contains(dataModel.getUsername())){
            viewHolder.action.setImageResource(R.drawable.remove_red);
        }
        else{
            viewHolder.action.setImageResource(R.drawable.add_red);
        }
